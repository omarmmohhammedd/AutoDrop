...
var RestClient = function (method, uri, headers, body, callback) {
    var restClient = function (uri, headers, body, callback) {
        var $this = {
            ...
            type: function (type) {
                $this.header("Content-Type", does(type).contain("/") ? type : mime.lookup(type));
                return $this;
            },
            send: function (data) {
                var type = $this.options.headers[$this.hasHeader("content-type")];
                if ((is(data).a(Object) || is(data).a(Array)) &&
                ...
                function handleFormData(form) {
                    for (var i = 0; i < $this._multipart.length; i++) {
                        ...
                        if (item.attachment && is(item.value).a(String)) {
                            if (does(item.value).contain("http://") ||
                    ...
                    return handleFormData(form).submit({
                        ...
                        response.on("data", function (chunk) {
                            if (typeof chunk === "string")
                                ...
                                response.body += decoder.write(chunk);
                        });
...
function does(value) {
    ...
    return {
        contain: function (field) {
            if (is(value).a(String))
                return value.indexOf(field) > -1;
            if (is(value).a(Object))
                return value.hasOwnProperty(field);
            if (is(value).a(Array))
                return !!~arrayIndexOf(value, field);
            return false;
        },
    };
}
